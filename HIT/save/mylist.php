...
if($katid=="") {
	$page_title = 'Meine Gesamtliste (ohne Zusatzstoffe) '; 	
}
elseif ($katid != "") {
	$sql_get_kat_title = 
			'SELECT *
...
$table_all = '<table class="list1" style="margin:auto;">
				  <tr>	
					<th style="width:10px;font-weight:700;font-size:11px;padding:2px;" >Allg. Wert</th>
					<th style="width:10px;font-weight:700;font-size:12px;padding:2px;" >Mein Wert</th>
...
$kat_list = '<li> <a href="./mylist.php" style="font-weight:900;"> <u>ALLES</u> </a> </li>';   
while($row_kat_list = mysqli_fetch_array($result_get_kat_list)) {
	$kat_list.= '<li> <a href="./mylist.php?katid='.$row_kat_list['KatID'].'">'.$row_kat_list['KatName'].'</a> </li>';	
	}
...
if($katid == "") {  // --------- WENN KEINE Kategorie = Alles
	...
	'SELECT *
	 FROM lebensmittel
	 WHERE eigene_wertung != "" AND kat_id NOT LIKE "26"
	 ORDER BY `name` ASC' ; 
	...
	while($row_all = mysqli_fetch_assoc($result_get_list_all)) {
		...
		switch($row_all["histamin_uv"]) {
			...
			case "1": $uv_color ="class='wert_txt_orange'"; break;
		...
		switch($row_all["eigene_wertung"]) {
			...
			case "1": $uv_color_mine ="class='wert_bg_orange'"; break;
...
elseif ($katid == "26") { // --------- WENN Zusatzstoffe 
	...
	'SELECT *
	 FROM lebensmittel
	 WHERE eigene_wertung != "" AND kat_id LIKE "26" 
	 ORDER BY `name` ASC' ;  
	...
	'SELECT *
	 FROM lebensmittel
	 WHERE kat_id = "'.$katid.'" AND eigene_wertung != "" ';
	...
	while($row_katinfo = mysqli_fetch_assoc($result_get_list_katinfo)) {
		...
		$sql_get_kategorie = 
			'SELECT *
			 FROM kategorie
			 WHERE katid LIKE "'.$row_katinfo["kat_id"].'" '; 
			$result_get_kategorie = mysqli_query($connect_hit, $sql_get_kategorie);
			$row_kat = mysqli_fetch_assoc($result_get_kategorie);
