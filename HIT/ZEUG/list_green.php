...
$kat_list = '<li> <a href="./list_green.php" style="font-weight:900;"> <u>ALLES</u> </a> </li>';   
while($row_kat_list = mysqli_fetch_array($result_get_kat_list)) {
	$kat_list.= '<li> <a href="./list_green.php?katid='.$row_kat_list['KatID'].'">'.$row_kat_list['KatName'].'</a> </li>';	
	}
...
if($katid == "") {  // --------- WENN KEINE Kategorie = Alles
	...
	'SELECT *
	 FROM lebensmittel
	 WHERE kat_id NOT LIKE "26" AND ( histamin_uv LIKE "0" OR histamin_uv LIKE "1" )
	 ORDER BY `name` ASC'; 
...
elseif ($katid == "26") { // --------- WENN Zusatzstoffe  
	...
	'SELECT *
	 FROM lebensmittel
	 WHERE kat_id = "26"  AND ( histamin_uv LIKE "0" OR histamin_uv LIKE "1" )';
...
elseif($katid == "karenz") {  // WENN Karenz AUSGEWÄHLT
	...
	'SELECT *
	 FROM lebensmittel
	 WHERE karenz LIKE "0"  AND ( histamin_uv LIKE "0" OR histamin_uv LIKE "1" )' ; 
	...
	'SELECT *
	 FROM lebensmittel
	 WHERE kat_id = "'.$katid.'"  AND ( histamin_uv LIKE "0" OR histamin_uv LIKE "1" )';
